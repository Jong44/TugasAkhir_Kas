...
if(isset($_POST['submit'])){
    ...
    $sql1 = "INSERT INTO rekapitulasi( tanggal, jenis, masuk) VALUES ('$tanggal','Masuk','$jumlah')";
    $sql2 = "INSERT INTO uang_masuk(nama, tanggal, jumlah) VALUES('$nama', '$tanggal', '$jumlah')" ;
    mysqli_query($conn, $sql1 ) or die(mysqli_error($conn));
    mysqli_query($conn, $sql2 ) or die(mysqli_error($conn));
    if($sql1){
        echo '<script>alert("Berhasil menambahkan data."); document.location="../tampilan/uang-masuk.php";</script>';
    }else{
...
<body>
    ...
    <div class="row no-gutters">
        ...
        <div class="item form-group">
            ...
            <?php
                $sql_anggota=mysqli_query($conn,"SELECT * FROM anggota") or die (mysqli_error($conn));
                while($nama_anggota = mysqli_fetch_array($sql_anggota)) {
                    echo '<option value="'.$nama_anggota['nama'].'">'.$nama_anggota['nama'].'</option>';
                } 
            ?>
            </select>
            </div>
